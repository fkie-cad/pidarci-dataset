...
def get_results_for_file_content(content, operation):
    ...
    for function_match in re.finditer(r"func_(?P<original_constant>(neg)?\d+)\([^\)]*\)\s*\{(?P<block>[^\}]+)\}", content):
        ...
        if match := re.search(f"{operation_sign}\s*(?P<matched_constant>\-?(0x)?[0-9a-f]+)", function_body):
            correct = False
...
if __name__ == "__main__":
    result = get_results_for_path(pathlib.Path("path_with_ghidra_decompiled_output"))
    with open("ghidra_results.json", "w") as outfile:
